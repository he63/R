...
# 파일에서 데이터 읽기
nums <- scan("data/sample_num.txt") #scan은 numeric(숫자)로 된 것만 읽어옴
nums
word_ansi <- scan("data/sample_ansi.txt",what="") # what=""를 입력함으로 문자열도 읽어옴옴
word_ansi
words_utf8 <- scan("data/sample_utf8.txt", what="",encoding="UTF-8")
words_utf8
words_err <- scan("data/sample_utf8.txt", what="")  # encoding="UTF-8"을 주지 않으면 한글은 깨진다.
words_err
...
df2 <- read.table("data/product_click.log", stringsAsFactors = T) # header-T
str(df2)
head(df2)
summary(df2$V2)
